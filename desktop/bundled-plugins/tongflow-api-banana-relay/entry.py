...
TONGFLOW_SLOT_MODELS = {
    "image-gen": [
        ...
        "nano-banana-pro-4k", "nano-banana2-1k", "nano-banana2-2k",
        "nano-banana2-4k",
    ],
    "image-edit": [
        "nano-banana-pro-1k", "nano-banana-pro-2k",
...
def _base_url() -> str:
    value = _env("BANANA_BASE_URL", _env("OPENAI_BASE_URL", DEFAULT_BASE_URL)).rstrip("/")
...
def _api_key() -> str:
    value = _env("BANANA_API_KEY", _env("OPENAI_API_KEY"))
    if not value:
        raise RuntimeError("请在设置中填写 BANANA_API_KEY")
    return value


def _model() -> str:
    return _REQUEST_MODEL or _env("BANANA_IMAGE_MODEL", DEFAULT_MODEL)


def _timeout() -> int:
    try:
        return max(30, int(_env("BANANA_TIMEOUT", "600")))
    except ValueError:
        return 600


def _async_enabled() -> bool:
    return _env("BANANA_ASYNC", "true").lower() not in {"0", "false", "no", "off"}
...
def _size_for_model(model: str, size: str | None) -> str | None:
    """Fit a requested size to fixed 1K/2K/4K Banana SKUs."""
    if not size:
        return size
...
def _http(url: str, *, method: str = "GET", body: bytes | None = None,
    ...
    try:
        ...
        raise RuntimeError(f"香蕉 API HTTP {exc.code} ({url}): {detail or exc.reason}") from exc
    except URLError as exc:
        raise RuntimeError(f"无法连接香蕉 API ({url}): {exc.reason}") from exc
...
def _json(body: bytes) -> dict[str, Any]:
    try:
        ...
        raise RuntimeError(f"香蕉 API 返回的不是 JSON：{preview}") from exc
    if not isinstance(value, dict):
        raise RuntimeError("香蕉 API 返回格式错误：顶层不是对象")
    return value
...
def _await_result(endpoint: str, response: dict[str, Any]):
    ...
    if not isinstance(task_id, str) or not task_id:
        ...
        raise RuntimeError(f"香蕉 API 响应中没有图片或任务 id：{preview}")
    deadline = time.monotonic() + _timeout()
    while time.monotonic() < deadline:
        status_response = _json(_http(f"{endpoint}/{task_id}"))
        ...
        if status in {"failed", "error", "cancelled", "canceled"}:
            ...
            raise RuntimeError(f"香蕉图片任务失败：{detail}")
        time.sleep(2)
    raise RuntimeError(f"香蕉图片任务超时（{_timeout()} 秒），任务 id：{task_id}")
