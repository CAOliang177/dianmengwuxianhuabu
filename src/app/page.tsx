...
import { ArrowRight, Clock3, Layers3, Pencil, Plus, Sparkles, WandSparkles } from "lucide-react";
        ...
        <span aria-label={text}>
            {Array.from(text).map((letter, index) => (
                <span
                    ...
                    style={{ "--letter-color": LETTER_COLORS[index % LETTER_COLORS.length] } as CSSProperties}
                >
                    {letter}
                </span>
            ))}
...
export default function Home() {
    ...
    const beginRenameCanvas = (event: React.MouseEvent, canvas: CanvasHistoryItem) => {
                ...
                <header className="flex items-center justify-between rounded-2xl border border-white/[.08] bg-white/[.035] px-4 py-3 shadow-[0_16px_50px_rgba(0,0,0,.18)] backdrop-blur-xl sm:px-5">
                    <div className="flex items-center gap-3">
                        <div className="relative">
                            ...
                            <img src="/dianmeng-brand.png" alt="dianmeng" className="relative h-11 w-11 rounded-xl border border-white/10 object-cover" />
                        </div>
                        <div className="leading-tight">
                            <div className="text-base font-semibold tracking-[.08em] sm:text-lg">
                                <span className="bg-gradient-to-r from-white via-blue-100 to-amber-100 bg-clip-text text-transparent">dianmeng</span>
                                <span className="ml-1 text-white/90">无限画布</span>
                            </div>
                ...
                <section className="grid min-h-[520px] items-center gap-8 py-10 lg:grid-cols-[1.12fr_.72fr] lg:gap-14 lg:py-12">
                    <div className="max-w-[760px]">
                        <div className="mb-5 inline-flex items-center gap-2 rounded-full border border-blue-300/15 bg-blue-400/[.08] px-3 py-1.5 text-sm text-blue-100 shadow-[inset_0_1px_0_rgba(255,255,255,.08)]">
                            <Sparkles className="h-4 w-4 text-amber-200" /> 灵感、模型与画布，在这里自然连接
                        </div>
                        <p className="mb-3 text-sm font-medium uppercase tracking-[.32em] text-slate-500">Create beyond the frame</p>
                        <h1 className="text-[clamp(3rem,5.2vw,5.3rem)] font-semibold leading-[.98] tracking-[-.055em]">
                            <span className="block text-white/95"><InteractiveTitle text="让灵感在" /></span>
                        ...
                        <p className="mt-6 max-w-2xl text-base leading-8 text-slate-300 sm:text-lg">
                            ...
                            <span className="text-slate-500"> 不打断灵感，也不限制边界。</span>
                        </p>

                        <div className="mt-7 flex flex-wrap items-center gap-3">
                            <button type="button" onClick={newCanvas} className="home-primary-button group flex items-center gap-3 rounded-2xl px-6 py-3.5 text-base font-semibold">
                            ...
                            <div className="flex items-center gap-2 rounded-2xl border border-white/[.07] bg-white/[.025] px-4 py-3 text-sm text-slate-400">
                                <Layers3 className="h-4 w-4 text-blue-300" /> 节点连接
                                <span className="h-3 w-px bg-white/10" />
                                <WandSparkles className="h-4 w-4 text-amber-200" /> 多模型生成
                            </div>
                        </div>
                    </div>
                    ...
                    <div className="relative mx-auto w-full max-w-[410px]">
                        ...
                        <div className="home-visual-card relative aspect-[.94] overflow-hidden rounded-[36px] border border-white/10 bg-white/[.035] p-5 shadow-[0_35px_90px_rgba(0,0,0,.42)] backdrop-blur-xl">
                            ...
                            <div className="relative h-full overflow-hidden rounded-[28px] border border-white/[.08] bg-[#090f20]/85 p-7">
                                <img src="/dianmeng-brand.png" alt="dianmeng AI 无限画布" className="h-full w-full rounded-2xl object-cover opacity-95 transition duration-700 hover:scale-[1.025]" />
                            ...
                            <div className="absolute bottom-7 left-1/2 flex -translate-x-1/2 items-center gap-2 rounded-full border border-white/10 bg-black/45 px-3 py-1.5 text-[11px] text-slate-300 backdrop-blur">
                                <span className="h-1.5 w-1.5 animate-pulse rounded-full bg-emerald-400" /> 创意引擎已就绪
                            </div>
                        </div>
                    </div>
                </section>
                    ...
                    <div className="mb-5 flex items-end justify-between gap-4">
                        <div>
                            <div className="flex items-center gap-2 text-xl font-semibold tracking-tight">
                                <span className="flex h-9 w-9 items-center justify-center rounded-xl bg-blue-400/10 text-blue-200"><Clock3 className="h-4 w-4" /></span>
                                继续你的创作
                            </div>
                            <p className="mt-2 text-sm text-slate-500">历史画布会自动保存，点击卡片即可从上次的位置继续</p>
                        </div>
                        <span className="rounded-full border border-white/[.07] bg-white/[.035] px-3 py-1 text-xs text-slate-500">{history.length} 个画布</span>
                    </div>

                    {history.length === 0 ? (
                        <button type="button" onClick={newCanvas} className="home-shine group flex min-h-44 w-full flex-col items-center justify-center overflow-hidden rounded-2xl border border-dashed border-white/15 bg-white/[.02] text-slate-400 transition hover:border-blue-400/40 hover:bg-blue-400/[.04]">
                            <Plus className="mb-3 h-8 w-8 text-blue-300 transition duration-300 group-hover:rotate-90 group-hover:scale-110" />
                            <span className="font-medium text-slate-200">创建你的第一个画布</span>
                            <span className="mt-1 text-sm">从空白开始，连接属于你的灵感网络</span>
                        </button>
                    ) : (
                        <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
                            {history.map((canvas) => (
                                <div key={canvas.id} className="home-shine group relative overflow-hidden rounded-2xl border border-white/[.08] bg-white/[.035] text-left transition duration-300 hover:-translate-y-1 hover:border-blue-300/30 hover:bg-white/[.065] hover:shadow-[0_22px_55px_rgba(0,0,0,.3)]">
                                    <button type="button" onClick={() => openCanvas(canvas.id)} className="w-full text-left">
                                        <div className="relative h-32 overflow-hidden bg-[radial-gradient(circle_at_25%_30%,rgba(92,128,255,.32),transparent_26%),linear-gradient(135deg,#111a34,#0b1020)]">
                                            ...
                                            <svg className="absolute inset-0 h-full w-full" aria-hidden="true"><path d="M120 58 C172 58, 188 78, 238 78" fill="none" stroke="rgba(171,188,255,.55)" strokeWidth="2" strokeDasharray="5 5" /></svg>
                                            <div className="absolute bottom-3 right-3 rounded-full bg-black/40 px-2.5 py-1 text-xs text-slate-300 backdrop-blur">{canvas.nodeCount} 个节点</div>
                                        </div>
                                        <div className="p-4">
                                            <div className="truncate font-medium text-slate-100 transition group-hover:text-blue-200">{canvas.name || "未命名画布"}</div>
                                            <div className="mt-2 text-xs text-slate-500">最后编辑 {new Date(canvas.updatedAt).toLocaleString("zh-CN")}</div>
                                        </div>
                                    </button>
                                    <button type="button" title="重命名画布" aria-label={`重命名 ${canvas.name || "未命名画布"}`}
                                        ...
                                        onClick={(event) => beginRenameCanvas(event, canvas)}>
            ...
            <Dialog open={renameTarget !== null} onOpenChange={(open) => { if (!open) setRenameTarget(null); }}>
                <DialogContent className="border-white/15 bg-[#11182a] text-white sm:max-w-md">
                    <DialogHeader>
                        ...
                        <DialogDescription className="text-slate-400">输入新的画布名称，保存后会同步到历史画布。</DialogDescription>
                    </DialogHeader>
                    <form className="space-y-4" onSubmit={submitRenameCanvas}>
                        <input autoFocus maxLength={80} value={renameValue} onChange={(event) => setRenameValue(event.target.value)}
                        ...
                        <div className="flex justify-end gap-3">
                            <button type="button" onClick={() => setRenameTarget(null)} className="rounded-xl border border-white/15 px-4 py-2 text-sm text-slate-300 transition hover:bg-white/5">取消</button>
                            <button type="submit" disabled={!renameValue.trim()} className="rounded-xl bg-blue-500 px-4 py-2 text-sm font-medium text-white transition hover:bg-blue-400 disabled:cursor-not-allowed disabled:opacity-40">保存</button>
                        </div>
                    </form>
                </DialogContent>
            </Dialog>
